...
var fs = require('fs');
  ...
  grunt.registerTask('mvn:preprocess', function (target) {
    ...
    grunt.config.set("template.version-js.options.data.version", grunt.config.get("mvn.version"));
    grunt.task.run('template:version-js');
  });
  ...
  grunt.registerTask('mvn:tag-release', function() {
    ...
    var releaseName = (pkg.name + "-" + pkg.version);
